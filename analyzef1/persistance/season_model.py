...
class Season(_DECL_BASE):
    ...
    id = Column(Integer, primary_key = True)
    session_result = relationship('SessionResult', order_by='SessionResult.id', cascade="all, delete-orphan")
    round_number = Column(Integer, nullable = False)
    ...
    def check_new_session():
        ...
        for event in df.itertuples(name='Event'):
            if Season.query.filter(Season.round_number == event.RoundNumber).first() == None:
                logger.info(f"RoundNumber {event.RoundNumber} not in database, adding now ...")
                new_event = Season(
                ...
                if event.EventFormat == 'sprint':
                    session_sprint = ff1.get_session(datetime.now().year, event.RoundNumber, 'S')
                    session_sprint.load()
                ...
                session_race = ff1.get_session(datetime.now().year, event.RoundNumber, 'R')
                session_race.load()
