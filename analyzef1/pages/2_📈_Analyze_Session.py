...
from analyzef1.constants import (
    CHOOSE_SESSION,
    DRIVER_TEAM_MAPPING,
    NAVBAR,
    YEAR,
...
def choose_session() -> Dict:
    ...
    data = {"year": year_input, "location": location_input, "event": session_input}
    return data
...
def app() -> None:
    ...
    with analyze_session[0]:
        ...
        driver_1 = col1.selectbox(
            "First Driver", get_driver_abbreviation(DRIVER_TEAM_MAPPING)
        )
        driver_2 = col2.selectbox(
            "Second Driver", get_driver_abbreviation(DRIVER_TEAM_MAPPING), index=1
        )
    ...
    with analyze_session[1]:
        ...
        driver_select = col2.selectbox(
            "Select Driver", get_driver_abbreviation(DRIVER_TEAM_MAPPING)
        )
